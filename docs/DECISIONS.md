...
| M10 | **A march is a trajectory first, a route second.** A regiment works out the line to where it was sent and follows it. The pathfinder is consulted only when that line meets **ground it cannot cross** — pathing is the exception, not the standing method. Friendly regiments are explicitly *not* a reason to re-path: they are dealt with by steering ([M13](#moving)), because a route planned around a body of men is stale by the time it is walked. | ❌ new |
| M11 | **Routes are rebuilt on a cadence, never every tick.** Re-planning is the most expensive thing the rules do, and a regiment that re-plans continuously is both slow and unsteady — it never commits to anything for long enough to get anywhere. | ⚠️ `RepathIntervalTicks = 5` exists and is being bypassed — finding 7 |
| M12 | **The rectangle pathfinds, not a point.** A route is only a route if the regiment's own width fits along it. Planning at 2 m clearance for a body 40 m wide finds gaps that do not exist and then blames whatever is standing in one. | ❌ new |
| M13 | **A regiment can sidestep.** It may crab along its own frontage without turning, at the same penalty a reversed march pays, to thread a gap between friends or to give ground sideways. This is what makes a line able to pass a hole narrower than it is: moving along its front, a 40 m by 9 m body sweeps 9 m, not 40. | ❌ new |
| M14 | **Plan wide, crab locally.** The planner only returns routes the regiment fits along facing forwards; sidestepping is a local behaviour for squeezing past friends and making room, not something the route promises. The regiment therefore walks round a gap it could physically have threaded — accepted for now, because a route that can only be walked sideways is a route that has to be re-checked every tick. **Deliberately left open**: planning the crabbing is a wanted improvement, not a rejected one ([T1](#later--technique)). | ❌ new |
